			...
			<div class="fixed inset-0 z-100 flex items-center justify-center p-4 backdrop-blur-sm">
				...
				>
					<div class="py-5 px-8 border-b bg-base-100 flex justify-between items-center">
						<div class="flex items-center gap-4">
							...
							<div>
								...
								<p class="text-[10px] font-bold text-primary tracking-widest uppercase">Registro de Auditoría de Operatividad / 稼働率詳細ログ</p>
							</div>
						</div>
							...
							<div class="lg:col-span-3 bg-base-100  rounded-xl border border-base-300 p-6 shadow-sm">
								<div class="mb-4 flex items-center justify-between">
									<span class="text-[10px] font-bold text-base-content/40 uppercase tracking-widest"
										>Registro de Auditoría de Operatividad / 稼働率詳細ログ</span
									>
										...
										<table class="w-full text-left text-xs border-collapse">
											<thead class="bg-base-200 sticky top-0 z-10 border-b border-base-300">
												<tr class="text-[9px] uppercase text-base-content/60 font-bold">
													<th class="px-4 py-3">Fecha / 日付</th>
													<!-- <th class="px-4 py-3 text-center">Trabajo (min)</th>
					...
					<div class="px-8 py-5 bg-base-100 border-t border-base-300 flex justify-end items-center gap-4">
						...
						<button (click)="close()" class="dbtn dbtn-sm dbtn-neutral text-[10px] font-bold px-8">CERRAR / 閉じる</button>
					</div>
				</div>
			</div>
		}
...
export class EffDetailModalComponent {
	...
	processedRecords = computed(() => {
		...
		const map = new Map<string, { work: number; total: number; operSum: number; count: number }>();

		for (const r of d.records) {
			const date = r.date.split('T')[0];
			const current = map.get(date) || { work: 0, total: 0, operSum: 0, count: 0 };
			current.work += r.work;
			current.total += r.total;
			current.operSum += r.oper;
			current.count++;
		...
		return Array.from(map.entries())
			...
			.map(([date, values]) => ({
				...
				work: values.work,
				total: values.total,
				oper: values.count > 0 ? values.operSum / values.count : 0,
			}));
	});
	...
	chartOptions = computed<any>(() => {
		...
		return {
			series: [
				{ name: 'Operatividad %', type: 'column', data: rows.map((r) => Math.round(r.oper)) },
				{ name: 'Meta', type: 'line', data: rows.map((r) => 85) },
			],
			chart: {
				type: 'line',
				height: 350,
